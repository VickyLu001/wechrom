...
def test_generate_naked_memory_files():
    ...
    try:
        prepare_memory.generate_naked_memory_files(memory_dir=dir, memory_filename=filename)
...
def test_get_naked_memory_template_bonds():
	...
	try:
		prepare_memory.generate_naked_memory_files(memory_dir=dir, memory_filename=filename)
		template_positions = prepare_memory.get_naked_memory_template_bonds(memory_list_dir = dir, memory_list_filename = filename)
...
def test_generate_nucleo_memory_files():
    # test if the memory list file is generated
    outfile_handle, outfile_path = tempfile.mkstemp()
    os.close(outfile_handle)
    dir, filename = os.path.split(outfile_path)
    try:
        prepare_memory.generate_nucleo_memory_files(memory_dir=dir, memory_filename=filename)
        with open(outfile_path, 'r') as fi:
            contents = fi.read()
    finally:
        os.remove(outfile_path)
